...
parser.add_argument("--num_layers", default=201, type=int,
                    help="Number of layers on DenseNet")
...
if __name__ == "__main__":
    ...
    # layer number check
    assert args.num_layers in (121, 169, 201, 264), "Invalid layer number!"
    ...
    model = densenet(args.num_layers)
    ...
    for epoch in range(args.epochs):
        ...
        if (val_loss < best_val_loss):
            torch.save({"epoch": epoch + 1,
                       ...
                       os.path.join(args.save_dir, f"densenet_{args.num_layers}_best.tar"))
